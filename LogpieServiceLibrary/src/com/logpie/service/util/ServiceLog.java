...
{
    private static boolean sDebug = ServiceConfig.isDebugging;
    private static String sPath;
    private static final String LOG_TAG = ServiceLog.class.getName();
    ...
    public synchronized static void writeFile(String TAG, String info)
    {
        ...
        try
        {
            ...
            writer.write(TAG + ": ");
            writer.write(info);
            writer.write(ServiceConfig.changeLineCharacter);
            writer.flush();
            writer.close();
        } catch (IOException e)
        {
            ServiceLog.e(LOG_TAG, "<Logpie> IOException: When writing log file.",e);
        }
    }
}
