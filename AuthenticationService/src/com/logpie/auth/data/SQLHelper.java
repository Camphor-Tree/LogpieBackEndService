...
public class SQLHelper
{
    ...
    {
        StringBuilder sqlBuilder = new StringBuilder();
        sqlBuilder.append("insert into user_auth (email,password) values (\'");
        sqlBuilder.append(email);
        sqlBuilder.append("\',\'");
    ...
    public static ArrayList<String> buildCreateUserStep2SQL(String uid, List<Scope> scopes)
    {
        ...
        String sql = String
                .format("update user_auth set access_token = '%s', access_token_expire_time = now()+ interval '1hour', refresh_token = '%s', refresh_token_expire_time = now() + interval '365days' where uid = %s;",
    ...
    public static String buildUpdateTokenSQL(String uid, String token)
    {
        ...
        String refresh_keysource = TokenScopeManager.addScope(
                TokenGenerator.generateRefreshTokenBaseKeySource(uid), scopeList);
     ...
     * @param uid
     * @param token
     * @return ArrayList<String> the first element is the sql, and the second
     *         element is access_token
     */
